...
\documentclass[
    master,     % 必选项：   {master, doctor} 填写学历水平
                % Mandatory: {master, doctor} Fill in the level of your degree
    professional,   % 必选项：   {academic, professional} 填写学位类型
                % Mandatory: {academic, professional} Fill in the type of your degree
...
% 第一个参数为中文标题，第二个参数为英文标题
% The first argument is Chinese title, the second argument is English title, if you have only English title, put it as both first and second argument
% 第二个为论文题目的英文翻译，请注意单词的大小写规律，如Structural Design and Rapid Development of Labyrinth Drip Irrigation Emitters
% The second argument is English translation of the Chinese title, please follow the capitalization rules of English. For example, "Structural Design and Rapid Development of Labyrinth Drip Irrigation Emitters". If you have only English title, you can ignore this tips.
\title{复杂观测环境下强化学习表征优化与策略泛化研究}{Research on Representation Optimization and Policy Generalization in Reinforcement Learning under Complex Observation Environments}

% 学位类型
% Type of your degree, Translate it from documents in `Materials/Requirements/2025/01 中英文题名页示例/英文标准翻译/'
% \degree{硕士}{Philosophy} % 学术型(Academic)硕士请基于 '学术学位名称.txt' 填写
...
% ==== 设置合作导师
% 注：以下情况可认定为申请学生的合作导师。
%（1）与招生简章中一致的合作导师；（2）CSC项目的合作导师；（3）交叉培养项目认定的合作导师。
% \advisorassociate{XXX}{教授}{XXX}{Prof.}

% ==== 设置合作导师
% 注：“导师团队”的书写需与招生简章保持一致
% The English Name of the Team must be consistent with the enrollment brochure
% \advisorteam{XXX教授团队\ 或\ “团队名称”}{English Name of the Team}

% 填写学术型学位的学科名称 / 专业型学位类别（领域）
% 请按照 '\Materials\Requirements\2025\01 中英文题名页示例' 下的附件1或附件2填写
% 英文翻译请参照 'Materials/Requirements/2025/01 中英文题名页示例/英文标准翻译/学科（专业）英文标准翻译.pdf'
% Name of the subject, also get it from that file
% Please fill in according to Attachment 1 or Attachment 2 under "\Materials\Requirements\2025\01 Sample of Chinese and English Title Pages"
% 
\subject{XXXX}{XXXX}
...
% 答辩委员会委员 显示的顺序和这里的一样 第一个人是主席 格式：单位, 职称, 姓名
...
\addcommitteemember{XXXXXXXXXX大学,XXX,XXX}
\addcommitteemember{XXXXXXXXXX大学,XXX,XXX}
\addcommitteemember{XXXXXXXXXX大学,XXX,XXX}
\addcommitteemember{XXXXXXXXXX大学,XXX,XXX}
\addcommitteemember{XXXXXXXXXX大学,XXX,XXX}
...
\defensedate{XXXX}{X}{X}

% 答辩地点（涉密论文请手动设置为「西安交通大学」）
% Defence location, default value is 「西安交通大学」
% 非涉密论文填写具体地点，如：西安交通大学主楼E座303室。
\defenseloc{XXXXXX}

% 常规审阅人 要求和答辩委员会委员一样
% General reviewer list, same requirements as the committee member
\addgeneralreviewer{西安交通大学,王XX,教授}
\addgeneralreviewer{XXXXXX大学,李XX,教授}
\addgeneralreviewer{XXXXXX大学,田XX,教授}
...
% 本部分内容非强制性要求，如果论文中所用符号不多，可以省略《主要符号表》。
% This section is not a mandatory requirement. If the number of symbols used in the paper is not large, the "Main Symbol Table" can be omitted.
% \thesisglossarylist
